...
class CotisTable extends Table {
    ...
    public function selectCotisConsult($pnm, $commune) {
        ...
        $query = $query . " ORDER BY id_pnm, u.nom, u.prenom";
        // var_dump($query);
        // echo '<br>' . $query . '<br>';
        return $this->query($query);
    }
    ...
    public function exportCotis($periodeDebut, $periodeFin, $membre_type = "", $pnm = 0) {
        // var_dump($_REQUEST);
        $query = "SELECT 
                  pnm.titre_pnm as Antenne,
                  CONCAT(m.civilite, ' ', u.prenom, ' ',u.nom) AS Membre, 
                  DATE_FORMAT(c.date_cotis, '%d/%m/%Y') as 'Paiement prevu', 
                  DATE_FORMAT(c.date_cotis_valid, '%d/%m/%Y') as 'Date du paiement',
                  DATE_FORMAT(DATE_ADD(c.date_cotis_valid, INTERVAL 1 year), '%d/%m/%Y') as `Renouvellement`, 
                  com.nom as Commune, 
                  m.membre_type AS Type, 
                  m.actif AS Actif 
                  FROM cotis c
                  JOIN membres m on c.id_user = m.users_id 
                  JOIN users u on c.id_user = u.id 
                  JOIN adresses a on m.adresse = a.id 
                  JOIN commune com on a.commune = com.id 
                  JOIN pnm on pnm.id_pnm = com.pnm
                  WHERE c.date_cotis >= '{$periodeDebut}'
                  AND c.date_cotis <= '{$periodeFin}'"; 
        if ($membre_type != '')
        {
            $query = $query . " AND m.membre_type = '" . $membre_type . "' ";
        }
        ...
        $query = $query . " order BY pnm.titre_pnm, c.date_cotis, u.nom, u.prenom";
